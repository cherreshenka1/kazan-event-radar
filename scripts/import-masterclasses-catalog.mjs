...
const itemManifest = readJson(projectPath("config", "catalog-masterclasses-items.json"));
...
const section = BASE_CATALOG.masterclasses;
if (!section?.items?.length) {
  throw new Error("Masterclasses section is empty in src/data/catalog.js");
}
...
for (const item of section.items) {
  ...
  normalizedItems.push({
    ...
    reviewUrl,
    reviewSource: "Яндекс",
    priceLabel: item.priceLabel || manifestEntry?.priceLabel || "",
    ...
    importMeta: {
      ...
      sourcePriority: ["officialSite", "guide", "map", "reviews"],
...
console.log(`Masterclass items normalized: ${normalizedItems.length}`);
...
if (missingManifestIds.length) {
  ...
  console.log("All masterclass items have manifest entries.");
}
