...
const itemManifest = readJson(projectPath("config", "catalog-active-items.json"));
...
const section = CATALOG.active;
if (!section?.items?.length) {
  throw new Error("Active section is empty in src/data/catalog.js");
}
...
console.log(`Active items normalized: ${normalizedItems.length}`);
...
if (missingManifestIds.length) {
  ...
  console.log("All active items have manifest entries.");
}
