...
const sectionId = "sights";

const catalogSources = readJson(projectPath("config", "catalog-sources.json"));
const itemManifest = readJson(projectPath("config", "catalog-sights-items.json"));
...
const section = CATALOG.sights;
if (!section?.items?.length) {
  throw new Error("Sights section is empty in src/data/catalog.js");
}
...
console.log(`Sight items normalized: ${normalizedItems.length}`);
...
if (missingManifestIds.length) {
  ...
  console.log("All sight items have manifest entries.");
}
