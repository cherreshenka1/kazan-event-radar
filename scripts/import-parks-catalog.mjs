...
const sectionId = "parks";

const catalogSources = readJson(projectPath("config", "catalog-sources.json"));
const itemManifest = readJson(projectPath("config", "catalog-parks-items.json"));
...
const section = CATALOG.parks;
if (!section?.items?.length) {
  throw new Error("Parks section is empty in src/data/catalog.js");
}
...
for (const item of section.items) {
  ...
  normalizedItems.push({
    ...
    sourceUrl: sourceRefs.guide || item.sourceUrl || "",
    sourceRefs,
    ...
    importMeta: {
      ...
      sourcePriority: ["guide", "map", "reviews"],
...
console.log(`Park items normalized: ${normalizedItems.length}`);
...
if (missingManifestIds.length) {
  ...
  console.log("All park items have manifest entries.");
}
